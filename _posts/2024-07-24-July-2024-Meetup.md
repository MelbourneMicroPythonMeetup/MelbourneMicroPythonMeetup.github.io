...
One of the more recent noteworthy features is that there is now an [experimental version of ViperIDE](https://viper-ide.org/?vm=1) that simulates a device
*inside the browser* using the MicroPython WASM build. Very neat! In my own brief testing since last month it seems things like variable suggestions are much improved, although make sure you save regularly as it seems like auto-save still isn't implemented.
